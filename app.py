...
RECOVERY_WAIT_TIMEOUT = float(os.environ.get("DLLAMA_RECOVERY_WAIT_TIMEOUT", "4.0"))
RECOVERY_STABLE_SEC = float(os.environ.get("DLLAMA_RECOVERY_STABLE_SEC", "0.4"))
...
def run_with_recovery(user_id: str, prompt: str) -> InferenceResult:
    ...
    for attempt in range(MAX_RECOVERY_RETRIES + 1):
        ...
        # 原本的 inference 停掉後，其他 worker 可能也會跟著重啟。
        # 這裡等到至少一個 worker 穩定 READY，再用新的 topology 重跑。
